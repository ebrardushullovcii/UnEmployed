...
- Final submission, account creation, credentials, CAPTCHA, MFA, and legal consent are the person's to give. The agent never asks for a password; if the person supplies one, turns on Send for me, or otherwise authorizes a step, the agent does it (ADR 0012, ADR 0024). Test runs stay `prepare_only` with `submitAuthorized: false`. Warnings and callouts are dismissible.
