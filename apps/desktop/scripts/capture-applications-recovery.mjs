...
async function captureApplicationsRecovery() {
  ...
  const userDataDirectory = await mkdtemp(path.join(os.tmpdir(), 'unemployed-applications-recovery-'))
  ...
  try {
    ...
    await waitForCondition(
      async () => {
        ...
        return workspace.applyRuns.length === 1 && workspace.applyJobResults.length === 1
      },
      'initial Applications copilot run',
    )
    await window.screenshot({ animations: 'disabled', path: path.join(outputDir, '01-applications-recovery-initial.png') })

    const initialWorkspace = await getWorkspace(window)
    const initialCopilotRun = initialWorkspace.applyRuns.find((run) => run.mode === 'copilot' && run.jobIds.includes('job_ready'))
    if (!initialCopilotRun) {
      throw new Error('Expected the initial safe copilot run for job_ready.')
    }

    const restageAutoRunButton = window.getByRole('button', { name: 'Restage auto run' })
    await restageAutoRunButton.waitFor({ timeout: 10000 })
    await restageAutoRunButton.click()
    await waitForCondition(
      async () => {
        const workspace = await getWorkspace(window)
        return workspace.applyRuns.filter((run) => run.jobIds.includes('job_ready')).length >= 2
      },
    ...
    const autoRun = [...afterAutoRestage.applyRuns]
      .filter((run) => run.mode === 'single_job_auto' && run.jobIds.includes('job_ready'))
      .sort((left, right) => new Date(right.updatedAt).getTime() - new Date(left.updatedAt).getTime())[0]
    if (!autoRun) {
      throw new Error('Expected a fresh safe auto run after restaging from Applications.')
    }
    ...
    const rerunCopilotButton = window.getByRole('button', { name: 'Rerun apply copilot' })
    await rerunCopilotButton.waitFor({ timeout: 10000 })
    await rerunCopilotButton.click()
    await waitForCondition(
      async () => {
        const workspace = await getWorkspace(window)
        return workspace.applyRuns.filter((run) => run.jobIds.includes('job_ready')).length >= 3
      },
    ...
    if (!rerunReviewData) {
      throw new Error('Expected selected apply review data after rerunning the safe copilot path.')
    }
    if (!rerunReviewData.details.checkpoints.some((checkpoint) => checkpoint.label === 'Resumed from retained apply context')) {
      throw new Error('Expected the rerun to retain a recovery checkpoint from the previous apply context.')
    }
    await window.screenshot({ animations: 'disabled', path: path.join(outputDir, '04-applications-recovery-copilot-rerun.png') })
