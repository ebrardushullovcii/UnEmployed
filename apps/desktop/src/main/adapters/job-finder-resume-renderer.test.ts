...
describe('job finder resume renderer', () => {
  ...
  test('renders ATS-safe classic html without keyword section bleed and with escaping', () => {
    ...
    expect(html).toContain('h4 { display: grid; grid-template-columns: minmax(0, 1fr) max-content;')
    expect(html).toContain('.entry-meta { color: var(--muted); font-size: 0.84rem; font-weight: 500; justify-self: end; white-space: nowrap; }')
    ...
    const bulletOnlyCredentialsDocument: ResumeRenderDocument = {
      ...
      sections: credentialHeavyRenderDocument.sections.map((section) =>
        section.id === 'section_certifications'
          ? { ...section, entries: [], bullets: [{ id: 'cert_bullet', text: 'AWS Certified Developer' }] }
          : section.id === 'section_education'
          ? { ...section, entries: [], bullets: [{ id: 'edu_bullet', text: 'MSc Human Computer Interaction' }] }
          : section,
      ),
    }
  ...
  test('renders every shipped template with ATS-safe single-column structure', () => {
    for (const template of listLocalResumeTemplates()) {
      ...
      expect(html).toContain('@page')
      expect(html).toContain('grid-template-columns: 1fr;')
      const gridTemplateColumnValues = [...html.matchAll(/\.body-grid\s*\{[^}]*grid-template-columns:\s*([^;]+);/g)]
