...
function getRecoveryCopy(pathname: string, scope: 'app' | 'route'): RecoveryCopy {
  if (scope === 'app') {
    return {
...
export function JobFinderRouteErrorBoundary({ scope }: JobFinderRouteErrorBoundaryProps) {
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {copy.secondaryAction ? (
                <Button
                  className="normal-case tracking-normal"
                  onClick={() => {
                    void navigate(copy.secondaryAction!.path, { replace: true })
                  }}
                  type="button"
                  variant="outline"
                >
          ...
          <div className="border-t border-destructive/12 bg-(--workspace-state-card-bg-default) p-6 sm:p-8 lg:border-l lg:border-t-0 lg:p-10">
            <div className="grid gap-4">
              ...
              <div className="grid gap-3 text-sm leading-6 text-foreground-soft">
                <p>The exception was isolated to the current route, so you do not have to restart the whole desktop shell unless reload fails.</p>
                <p>Use the fallback action to move back to a stable screen, then reopen the workflow when you are ready.</p>
              </div>
            </div>
          </div>
        </div>
