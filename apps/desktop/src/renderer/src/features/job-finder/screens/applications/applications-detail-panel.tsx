...
interface ApplicationsDetailPanelProps {
  ...
  effectiveSelectedApplyResult: JobFinderWorkspaceSnapshot['applyJobResults'][number] | null;
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            {visibleApplyResult ? (
              <div className="surface-card-tint rounded-(--radius-field) border border-(--surface-panel-border) px-4 py-4">
                ...
                <p className="mt-2 text-(length:--text-small) leading-6 text-foreground-soft">
                  {visibleApplyResult.latestQuestionCount} questions • {visibleApplyResult.latestAnswerCount} grounded answers • {visibleApplyResult.artifactCount} retained artifacts
                </p>
          ...
          <section className="surface-card-tint grid gap-4 rounded-(--radius-field) border border-(--surface-panel-border) px-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <StatusBadge tone={visibleApplyResult ? 'active' : 'muted'}>
                {applyRunHistory.length} run{applyRunHistory.length === 1 ? '' : 's'} saved
              </StatusBadge>
            </div>
            ...
            <div className="grid gap-1 text-(length:--text-small) leading-6 text-foreground-soft">
              {!canRestageAutoRun ? (
                <p>
                  Auto-run restaging stays available only when this job is still in a review-ready stage.
                </p>
              ) : null}
              {selectedRun?.mode === 'queue_auto' ? (
                <div className="grid gap-3 rounded-(--radius-field) border border-(--surface-panel-border) bg-background/40 px-3 py-3">
                  <p>
                    Queue recovery will restage {selectedQueueRecoveryJobIds.length} remaining or blocked job
                    {selectedQueueRecoveryJobIds.length === 1 ? '' : 's'} from the selected queue run.
                  </p>
                  <div className="grid gap-2 md:grid-cols-2">
                    ...
                    <div className="grid gap-2">
                      <p className="label-mono-xs">Already completed or review-ready</p>
                      {excludedQueueRecoveryEntries.length ? (
                        excludedQueueRecoveryEntries.map((entry) => (
                          <div key={`exclude-${entry.jobId}`} className="flex flex-wrap items-center justify-between gap-2 rounded-(--radius-field) border border-(--surface-panel-border) bg-background/50 px-3 py-2">
                            <span className="text-foreground">{entry.label}</span>
                            <StatusBadge tone={getQueueRecoveryTone(entry.runResult?.state ?? null)}>
                              {formatStatusLabel(entry.runResult?.state ?? 'awaiting_review')}
                            </StatusBadge>
                          </div>
                        ))
                      ) : (
                        <p>No jobs are excluded from this historical queue yet.</p>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <section className="surface-card-tint grid gap-4 rounded-(--radius-field) border border-(--surface-panel-border) px-4 py-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="grid gap-1">
                  ...
                  <p className="text-(length:--text-small) leading-6 text-foreground-soft">
                    Review how each job in the selected historical queue finished before you restage anything.
                  </p>
                </div>
                <StatusBadge tone={canRestageQueueRun ? 'active' : 'muted'}>
                  {selectedQueueOutcomeEntries.length} job{selectedQueueOutcomeEntries.length === 1 ? '' : 's'} in run
                </StatusBadge>
              </div>
              <p className="text-(length:--text-small) leading-6 text-foreground-soft">
              ...
              <div className="grid gap-2">
                {selectedQueueOutcomeEntries.map((entry) => (
                  ...
                  >
                    ...
                    <p className="text-(length:--text-small) leading-6 text-foreground-soft">
                      {entry.runResult?.summary ?? 'This job never started before the queue paused or was cancelled.'}
                    </p>
            ...
            <section className="surface-card-tint grid gap-4 rounded-(--radius-field) border border-(--surface-panel-border) px-4 py-4">
              ...
              {applyRunDetailsStatus === 'loading' ? (
                <p className="text-(length:--text-body) leading-7 text-foreground-soft">
                  Loading persisted questions, grounded answers, artifacts, and checkpoints for this apply run.
                </p>
              ) : null}
                      ...
                      {applyRunDetails.questionRecords.map((question) => (
                        <div key={question.id} className="rounded-(--radius-field) border border-(--surface-panel-border) bg-background/40 px-3 py-3">
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            ...
                            <StatusBadge tone={question.status === 'submitted' || question.status === 'answered' ? 'positive' : question.status === 'skipped' ? 'critical' : 'active'}>
                  ...
                  {applyRunDetails.answerRecords.length ? (
                    <div className="grid gap-2">
                      ...
                      {applyRunDetails.answerRecords.map((answer) => (
                        <div key={answer.id} className="rounded-(--radius-field) border border-(--surface-panel-border) bg-background/40 px-3 py-3">
                          <div className="flex flex-wrap items-center justify-between gap-2">
                          ...
                          <p className="mt-2 text-(length:--text-small) leading-6 text-foreground-soft">
                            {formatStatusLabel(answer.sourceKind)} source{answer.confidenceLabel ? ` • ${answer.confidenceLabel}` : ''}
                          </p>
                  ...
                  {applyRunDetails.artifactRefs.length ? (
                    <div className="grid gap-2">
                      ...
                      {applyRunDetails.artifactRefs.map((artifact) => (
                        <div key={artifact.id} className="rounded-(--radius-field) border border-(--surface-panel-border) bg-background/40 px-3 py-3 text-(length:--text-small) leading-6 text-foreground-soft">
                          ...
                          {artifact.storagePath ? <p className="break-all">Saved: {artifact.storagePath}</p> : null}
                  ...
                  {applyRunDetails.checkpoints.length ? (
                    <div className="grid gap-2">
                      ...
                      {applyRunDetails.checkpoints.map((checkpoint) => (
                        <div key={checkpoint.id} className="rounded-(--radius-field) border border-(--surface-panel-border) bg-background/40 px-3 py-3 text-(length:--text-small) leading-6 text-foreground-soft">
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <strong className="text-foreground">{checkpoint.label}</strong>
                            <StatusBadge tone={checkpoint.jobState === 'submitted' ? 'positive' : checkpoint.jobState === 'blocked' || checkpoint.jobState === 'failed' ? 'critical' : 'active'}>
                  ...
                  {applyRunDetails.consentRequests.length ? (
                    <div className="grid gap-2">
                      ...
                      {applyRunDetails.consentRequests.map((request) => (
                        <div key={request.id} className="rounded-(--radius-field) border border-(--surface-panel-border) bg-background/40 px-3 py-3 text-(length:--text-small) leading-6 text-foreground-soft">
                          <div className="flex flex-wrap items-center justify-between gap-2">
