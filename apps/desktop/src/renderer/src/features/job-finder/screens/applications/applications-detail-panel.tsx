            ...
            <div className="grid gap-0">
              {selectedRecord.events.map((event) => {
                ...
                <article key={event.id} className="relative grid gap-3 border-l border-border/20 pl-8 pb-8 sm:grid-cols-[1fr]">
                   <div className={cn('absolute left-[-5px] top-1 h-2.5 w-2.5', tone === 'positive' ? 'bg-positive' : tone === 'active' ? 'bg-primary' : tone === 'critical' ? 'bg-destructive' : 'border border-border bg-background')} />
                   <div>
                     <div className="label-mono-xs">{formatTimestamp(event.at)}</div>
                     <strong className={cn('mt-1 block text-sm font-medium', tone === 'positive' ? 'text-positive' : tone === 'active' ? 'text-primary' : tone === 'critical' ? 'text-destructive' : 'text-foreground')}>{event.title}</strong>
