    ...
    <aside className="surface-panel-shell relative grid min-h-0 min-w-0 gap-3 overflow-hidden rounded-(--radius-field) border border-(--surface-panel-border) p-3.5">
      ...
      <div className="grid min-h-0 gap-3 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.2fr)_minmax(0,0.85fr)]">
        ...
        <div className="surface-card-tint grid min-w-0 gap-2 rounded-(--radius-field) border border-(--surface-panel-border) p-4">
          ...
          {profileSummary ? <p className="text-sm leading-6 text-foreground-soft">{profileSummary}</p> : null}
          {highlightedProof ? (
            <div className="grid gap-1 text-sm text-foreground-soft">
              <strong className="text-foreground">{highlightedProof.title}</strong>
              <p>{truncateText(highlightedProof.claim, 110)}</p>
              {highlightedProof.heroMetric ? <p>Metric: {highlightedProof.heroMetric}</p> : null}
            </div>
          ) : null}
          {!profileSummary && !highlightedProof ? <p className="text-sm text-foreground-soft">No reusable profile context saved yet.</p> : null}
        </div>
        ...
        <div className="surface-card-tint grid min-w-0 gap-2 rounded-(--radius-field) border border-(--surface-panel-border) p-4">
          ...
          <p className="text-sm text-foreground-soft">
            {research.length > 0 ? `${research.length} saved source${research.length === 1 ? '' : 's'}.` : 'No research saved yet.'}
          </p>
          {leadResearch ? (
            <div className="grid gap-1 text-sm text-foreground-soft">
              <strong className="text-foreground">{truncateText(leadResearch.pageTitle ?? leadResearch.sourceUrl, 72)}</strong>
