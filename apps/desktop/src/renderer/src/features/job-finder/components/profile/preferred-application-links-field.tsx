...
function linkDisplayName(link: ProfileEditorValues['links'][number], index: number): string {
  const label = link.label.trim()
  if (label) {
    return label
  }

  return link.kind ? `${link.kind.charAt(0).toUpperCase()}${link.kind.slice(1)} link` : `Public link ${index + 1}`
}
  ...
  return (
    <Controller
      ...
      render={({ field }) => {
        ...
        return (
          <fieldset className="grid min-w-0 gap-3 md:col-span-2" id={props.fieldId}>
            <legend className="text-sm font-medium text-foreground">Links to include on applications</legend>
            <p className="text-sm leading-6 text-foreground-soft">
              Choose from the public links saved in Background. The application will use the label and URL, never an internal ID.
            </p>
            {links.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="min-w-0">
                        <span className="block text-sm font-semibold text-foreground">{linkDisplayName(link, index)}</span>
                        <span className="block break-all text-sm leading-6 text-foreground-soft">
                          {link.url.trim() || 'Add this link’s URL in Background before selecting it.'}
                        </span>
                      </span>
                    </label>
                  )
              ...
              <p className="rounded-(--radius-field) border border-dashed border-border/45 p-3 text-sm leading-6 text-foreground-soft">
                No public links are saved yet. Add LinkedIn, a portfolio, or another profile in Background first.
              </p>
            )}
          </fieldset>
        )
