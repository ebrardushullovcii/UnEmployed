...
const profileFocusTargets: Record<ProfileDeepLinkFocus, { headingId: string; sectionId: string }> = {
  'job-sources': {
...
export function focusProfileDeepLink(
  ...
  const sectionScroller = documentRef.getElementById(PROFILE_SECTION_SCROLL_AREA_ID)

  if (!heading || !section || !(sectionScroller instanceof HTMLElement)) {
    return false
  }

  const pageScroller = sectionScroller.closest('.screen-scroll-area')
  if (pageScroller instanceof HTMLElement) {
    resetScroll(pageScroller, 0)
  }
