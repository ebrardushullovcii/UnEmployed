...
function buildFamilyViewModels(
  ...
  return [...families.values()]
    ...
    .sort((left, right) => {
      return (
        left.familySortOrder - right.familySortOrder ||
...
export function ResumeThemePicker({
  ...
  const focusedFamily =
    families.find((family) => family.id === focusedFamilyId) ??
    families[0] ??
    null;
  const focusedFamilyRecommended =
    focusedFamily?.templates.find((template) =>
              ...
              <div className="grid gap-2 rounded-(--radius-field) border border-(--surface-panel-border) bg-background/55 px-3 py-2.5">
                ...
                {selectedFamily || leadingRecommendedFamily ? (
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[0.72rem] leading-4 text-foreground-soft">
                    {selectedFamily ? (
                      <span>Current: {selectedFamily.label}</span>
                    ) : null}
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="text-[0.68rem] leading-4 text-foreground-soft">
                          {family.templates.length} variant
                          {family.templates.length === 1 ? "" : "s"}
                        </span>
                      </button>
