...
function ProtectionBadge({ state }: { state: InterviewOverlaySnapshot['protectionState'] }) {
...
export function AnswerCueOverlay(props: {
  ...
  return (
    <section
      className={cn(
        'overflow-hidden border border-(--surface-panel-border-warm) bg-black/72 text-foreground shadow-[0_24px_90px_rgba(0,0,0,0.48)] backdrop-blur-2xl',
        ...
        {cue ? (
          <>
            <div className="grid gap-2">
              ...
              <p className="text-[0.96rem] leading-6 text-foreground">{cue.question}</p>
            </div>
            ...
            <footer className="flex flex-wrap items-center gap-2 border-t border-border-subtle pt-3 text-[0.72rem] text-muted-foreground">
              ...
              {cue.disclosure.overlayContaminated ? <span>visual degraded</span> : null}
            </footer>
          </>
        ) : (
          <div className="grid min-h-56 place-items-center text-center">
            <div className="grid gap-2">
              ...
              <p className="text-[0.9rem] text-muted-foreground">No cue card yet.</p>
            </div>
          </div>
        )}
      </div>
...
export function TranscriptOverlay(props: {
  ...
  return (
    <section
      className={cn(
        'overflow-hidden border border-(--info-border) bg-black/76 text-foreground shadow-[0_24px_90px_rgba(0,0,0,0.48)] backdrop-blur-2xl',
        props.framed ? 'rounded-(--radius-panel)' : 'h-screen'
      )}
      style={{ opacity: props.snapshot.opacity }}
    >
      ...
      <div className="grid max-h-[28rem] gap-4 overflow-y-auto p-4">
        {props.snapshot.transcriptSegments.map((segment) => (
          <article key={segment.id} className="grid grid-cols-[4.5rem_minmax(0,1fr)] gap-3">
            ...
            <div className="grid gap-1">
              ...
              <p className="text-[0.86rem] leading-5 text-foreground-soft">{segment.text}</p>
            </div>
          </article>
        ))}
      </div>
