...
- current baseline: deterministic contracts/service/persistence, restart-to-`interrupted` recovery, desktop IPC/preload, top-level route, two separate answer/transcript overlay BrowserWindows, no main-window live cue/transcript mirroring, tray/global-hotkey semantic controls, post-session review/export/delete, Interview Helper UI harness, and Windows Electron capture-protection harness are landed; real platform audio/STT, meeting-app-specific capture exclusion, and macOS/Linux evidence remain active work
