...
- current baseline: deterministic contracts/service/persistence, desktop IPC/preload, top-level route, two separate answer/transcript overlay BrowserWindows, tray/global-hotkey semantic controls, post-session review/export/delete, and Interview Helper UI harness are landed; real platform audio/STT/capture-verification adapters and macOS/Linux evidence remain active work
