...
- status: `done`
- linked plan: `docs/exec-plans/completed/036-interview-helper-live-session-full-product.md`
- completed baseline: deterministic contracts/service/persistence, restart-to-`interrupted` recovery, desktop IPC/preload, top-level route, Job Finder saved-job and application-record setup handoff, explicit post-session Job Finder write-back actions, setup preferences, full rehearsal checks, per-session confirmation, paused reconfiguration, diagnostics, Electron-backed permission/capture readiness checks, source-labeled transcript ingestion, native-caption clipboard/file watchers, browser speech bridge, media probes and transient STT controls, local-command STT, OpenAI-compatible cue/transcription providers, schema validation and deterministic fallback, two protected overlay windows, layout persistence/reset, display-change protection revalidation, tray/global-hotkey semantic controls, post-session review/export/annotation/delete, no main-window live cue/transcript mirroring, and replayable desktop harnesses
- accepted exceptions: macOS/Linux hardware evidence requires actual target hosts, and meeting-app-specific caption/capture-exclusion integrations require an authorized platform path; the future full capture-exclusion architecture remains adapter-owned under `packages/os-integration`
