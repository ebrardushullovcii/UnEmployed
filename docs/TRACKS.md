...
- current baseline: deterministic contracts/service/persistence, restart-to-`interrupted` recovery, desktop IPC/preload, top-level route, Job Finder saved-job and application-record setup handoff, explicit post-session Job Finder write-back actions for linked application records, setup preferences for selected transcription language/cue sensitivity/automatic screenshot-on-cue, explicit full rehearsal checks for language, audio, transcription fallback, providers, screenshots, overlays, capture-protection request state, hotkeys/tray, and retention defaults, explicit per-session confirmation before capture start, explicit paused reconfiguration flow, structured diagnostics surface, Electron-backed microphone permission and desktop capture source readiness checks, typed source-labeled live transcript ingestion for microphone/meeting audio/meeting-native captions, user-started native-caption clipboard and caption-file watchers, user-started browser speech bridge where Chromium speech recognition is available, user-started temporary media stream probes plus transient audio transcription controls, configurable local-command STT for user-installed local engines, opted-in automatic visual batches for automatic transcript cues, environment-backed OpenAI-compatible cue-card generation and cloud audio transcription with schema validation, one retry before deterministic cue fallback, service-level quiet fallback cards for invalid cue output, two separate answer/transcript overlay BrowserWindows, persisted overlay layout/display metadata with reset, display-change stale marking plus automatic ordinary Electron protection revalidation, per-session ordinary Electron capture protection verification, no main-window live cue/transcript mirroring, tray/global-hotkey semantic controls, post-session review/export/transcript-annotation/delete, Interview Helper UI harness, and Windows Electron capture-protection harness are landed; meeting-app-specific caption APIs, meeting-app-specific capture exclusion, and macOS/Linux evidence remain active work
