...
- Live discovery and supported apply flows now only consume the newest draft or validated instruction artifact for the matching target: the current draft artifact applies automatically for that target, and validated artifacts apply when no newer draft has replaced them
