...
- Interview Helper is its own top-level module route. Job Finder can deep-link into setup with suggested saved-job or application-record context.
...
Validation failure produces a quiet visible failure card. Transient provider errors retry once with the configured short timeout before deterministic fallback.
...
- verification method
- display/window metadata and automatic stale-mark plus ordinary Electron revalidation after display changes
...
- add deep links from saved jobs and application records into Interview setup with suggested target context
...
- post-session review, export, prep-artifact conversion, annotation, explicit Job Finder application actions, and delete
...
- Landed integrated baseline: Interview Helper contracts, explicit setup-consent gate, setup preferences for selected transcription language/cue sensitivity/automatic screenshot-on-cue, explicit full rehearsal checks for language, audio, transcription fallback, providers, screenshots, overlays, capture-protection request state, hotkeys/tray, and retention defaults, explicit per-session confirmation before capture start, explicit paused reconfiguration flow, structured diagnostics surface, service state machine, file-backed workspace persistence, restart-to-`interrupted` recovery, Job Finder saved-job and application-record setup handoff, explicit post-session Job Finder write-back actions for linked application records, Electron-backed microphone permission and desktop capture source readiness checks, source-labeled live transcript ingestion for microphone/meeting audio/meeting-native captions, user-started native-caption clipboard and caption-file watchers, user-started browser speech bridge, temporary renderer media stream probes plus transient MediaRecorder audio transcription controls, configurable local-command STT for user-installed local engines, static protected-surface `os-integration` capability adapters, per-session ordinary Electron capture protection verification, display-change stale marking plus automatic ordinary Electron revalidation, deterministic vision/summary providers, deterministic and environment-backed OpenAI-compatible cue/transcription providers with schema validation, one retry before deterministic cue fallback, service-level quiet fallback cards for invalid cue output, opted-in automatic visual batches for automatic transcript cues, desktop main IPC/preload hosting, top-level route, separate answer/transcript overlay BrowserWindows, persisted overlay layout/display metadata with reset, no main-window live cue/transcript mirroring, tray/global-hotkey semantic controls, and additive post-session transcript annotations.
- Latest green evidence: `pnpm validate:contracts`; `pnpm validate:package interview-helper`; `pnpm validate:package os-integration`; `pnpm validate:package ai-providers`; `pnpm validate:package job-finder`; `pnpm validate:desktop`; `pnpm validate:docs-only`; `pnpm --filter @unemployed/desktop ui:interview-helper`; `pnpm --filter @unemployed/desktop ui:interview-helper-protection`; `pnpm verify:affected`.
- Harness artifacts: `apps/desktop/test-artifacts/ui/interview-helper/interview-helper-report.json` and `apps/desktop/test-artifacts/ui/interview-helper-protection/interview-helper-protection-report.json`; the desktop workflow now records Electron microphone permission status, enumerates Electron desktop capture sources for meeting/system readiness, verifies explicit per-session confirmation, verifies explicit rehearsal checks for language, transcription fallback, screenshot capture, screenshot vision, overlay protection request state, and retention defaults, verifies selected language/cue sensitivity/automatic screenshot setup persistence, verifies the paused reconfiguration flow, verifies structured diagnostics visibility, verifies typed meeting-native transcript ingestion can add a platform-local segment and generate a cue without mirroring that transcript in the main window, runs the active-session overlay protection verifier, captures optional visual context with Electron `desktopCapturer`, immediately deletes the temporary PNG, records only metadata plus overlay-contamination disclosure, persists moved overlay bounds with display ids, verifies overlay layout reset restores default bounds/display ids without deleting sessions, simulates an Electron display-metrics change and verifies overlay protection evidence is revalidated, verifies explicit post-session Job Finder mark-interviewed and follow-up-note actions for a linked application record, and Windows Electron `desktopCapturer` protection evidence did not detect protected overlay-window pixels. Both harnesses now fail on release-critical report invariant failures instead of only writing false report fields. Runtime verified protection remains scoped to the exact ordinary Electron capture evidence and does not prove meeting-app-specific exclusion.
