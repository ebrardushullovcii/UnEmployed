...
- one truthful shared catalog now models family, variant, delivery lane, ATS confidence, fit guidance, visual tags, ordering, and explicit apply, approval, and benchmark eligibility across the original six `026` shipped template ids; see the `031` extension note below for the current eight-template catalog
...
- the `026` family-first selection baseline has since been extended by `031`; the shipped `apply-safe` families now stay single-column and ATS-safe while covering `Chronology Classic`, `Senior Brief`, `Modern Editorial`, `Engineering Spec`, `Proof Portfolio`, `Credential Ledger`, `Longform Timeline`, and `Career Pivot Bridge`
