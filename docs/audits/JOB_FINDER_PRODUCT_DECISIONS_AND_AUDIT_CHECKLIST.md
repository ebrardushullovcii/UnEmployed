...
- [x] Computer Use reproduced the reported Profile Copilot defect at the 1024×720 minimum: the expanded panel covered essential setup fields and the collapsed bubble floated above the page bottom. The historical reserved-right-rail correction was later superseded because it made Profile look collapsed. Current behavior keeps Profile geometry unchanged and uses a compact modeless chat below the tabs, with minimize restoring any temporarily covered lower-right controls.
...
- [x] The Profile Copilot bubble supports full-distance native pointer dragging instead of stopping after the first small movement. Its collapsed drop point is clamped, returns after minimizing, and a later single click still expands it. The current compact expansion is modeless and does not reserve a form rail or change Profile width; acceptance compares closed/open Profile geometry and treats excessive visual occlusion as a failure.
