...
- Interview Helper native-caption clipboard intake is user-started and returns only the current clipboard text through typed preload IPC; renderer code must convert it into `meeting_native_transcript` segments through the same transcript ingestion contract.
