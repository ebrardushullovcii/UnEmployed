...
- Job posting, adapter-driven discovery targets with optional per-target custom navigation instructions, target-level source-instruction status/ids, source-debug run and attempt artifacts, evidence refs, retained discovery runs, activity timeline events, fit assessment, discovery provenance, and review queue items, plus learned navigation/search/detail/apply guidance artifacts for each target
...
- Job Finder repository/workspace snapshot state plus typed save/update IPC payloads for profile, preferences, settings, retained run history, staged discovery results, source-debug launch/query/cancel actions, source-instruction promotion/verification actions, and renderer-visible learned instruction artifacts
...
- Source-debug artifacts should stay structured and curated: persist attempt artifacts, evidence refs, phase summaries, and instruction artifacts instead of raw worker transcripts
