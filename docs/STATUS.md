...
Plan `007` implementation and hardening. Plan `008` remains the next queued follow-on slice, plan `009` is complete as the current shipped wording baseline after stronger rewrite and later structural polish passes, and plan `010` remains queued after that.
