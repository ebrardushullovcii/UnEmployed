...
- `036` is active: Interview Helper now has the first integrated deterministic desktop workflow: contracts, state machine, file-backed persistence, restart-to-`interrupted` recovery, main-process service hosting, typed preload APIs, a top-level route, separate answer/transcript overlay BrowserWindows with Electron content-protection requests, tray/global-hotkey semantic controls, post-session review/export/delete flows, and replayable desktop harness evidence. It is not complete as a cross-platform hardware release yet: real microphone and meeting/system audio capture, local/platform/cloud STT adapters, automated capture-exclusion verification, and macOS/Linux validation still need target-platform implementation/evidence. Future authorized full capture exclusion remains the highest-priority architectural constraint, so protected overlay surfaces must stay first-class `os-integration` capabilities with verification results instead of booleans or session-logic assumptions. Work from `docs/exec-plans/active/036-interview-helper-live-session-full-product.md`.
