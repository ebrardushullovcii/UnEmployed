...
- `036` is active: Interview Helper now has the first integrated deterministic desktop workflow: contracts, state machine, file-backed persistence, restart-to-`interrupted` recovery, main-process service hosting, typed preload APIs, a top-level route, Electron-backed microphone permission and desktop capture source readiness checks, Electron `desktopCapturer` screenshot context with immediate temporary-file cleanup, separate answer/transcript overlay BrowserWindows with Electron content-protection requests, persisted overlay layout/display metadata, tray/global-hotkey semantic controls, post-session review/export/transcript-annotation/delete flows, no live cue/transcript mirroring in the main window, and replayable desktop harness evidence. Windows Electron `desktopCapturer` protection evidence did not detect protected overlay pixels. It is not complete as a cross-platform hardware release yet: real microphone and meeting/system audio capture streams, local/platform/cloud STT adapters, meeting-app-specific capture exclusion, and macOS/Linux validation still need target-platform implementation/evidence. Future authorized full capture exclusion remains the highest-priority architectural constraint, so protected overlay surfaces must stay first-class `os-integration` capabilities with verification results instead of booleans or session-logic assumptions. Work from `docs/exec-plans/active/036-interview-helper-live-session-full-product.md`.
