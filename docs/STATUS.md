...
- Completed foundations: `007`, `009`, `010`, `011`, `012`, `013`, `014`, `015`, `016`, `017`, `018`, `019`, `020`, `021`, `022`, `023`, `024`, `025`, `026`, `027`, `028`, `029`, `030`, `031`, `032`, `033`, `034`, `035`, `036`
- Active work: none
...
- `036` is completed with accepted target-platform exceptions: Interview Helper now has the first integrated desktop workflow: contracts, state machine, file-backed persistence, restart-to-`interrupted` recovery, main-process service hosting, typed preload APIs, a top-level route, Job Finder saved-job and application-record setup handoff, explicit post-session Job Finder write-back actions for linked application records, setup preferences for selected transcription language, cue sensitivity, and automatic screenshot-on-cue, explicit rehearsal checks for language, audio, transcription fallback, providers, screenshots, overlays, capture-protection request state, hotkeys/tray, and structured retention defaults, explicit per-session confirmation before starting capture, explicit paused reconfiguration flow, structured diagnostics surface, Electron-backed microphone permission and desktop capture source readiness checks, source-labeled live transcript ingestion for microphone/meeting audio/meeting-native captions, user-started native-caption clipboard and caption-file watchers, a user-started browser speech bridge where Chromium speech recognition is available, user-started temporary media stream probes plus transient MediaRecorder audio transcription controls for microphone and display/system audio, configurable local-command STT for user-installed local engines, Electron `desktopCapturer` screenshot context with immediate temporary-file cleanup for manual and opted-in automatic cues, environment-backed OpenAI-compatible cue-card generation and cloud audio transcription with schema validation, cue-provider one-retry behavior, deterministic cue fallback, service-level quiet fallback cards for invalid cue output, separate answer/transcript overlay BrowserWindows with Electron content-protection requests, persisted overlay layout/display metadata with reset, display-change stale marking plus automatic ordinary Electron protection revalidation, per-session ordinary Electron capture protection verification, tray/global-hotkey semantic controls, post-session review/export/transcript-annotation/delete flows, no live cue/transcript mirroring in the main window, and replayable desktop harness evidence. Windows Electron `desktopCapturer` protection evidence did not detect protected overlay pixels. macOS/Linux hardware validation and meeting-app-specific caption/capture-exclusion integrations remain documented exceptions because they require target hosts or authorized platform paths unavailable in this workspace. Future authorized full capture exclusion remains the highest-priority architectural constraint, so protected overlay surfaces must stay first-class `os-integration` capabilities with verification results instead of booleans or session-logic assumptions. See `docs/exec-plans/completed/036-interview-helper-live-session-full-product.md`.
...
- No active plan is open. Reopen Interview Helper work only for a concrete regression, target-platform validation run, authorized meeting-platform integration, or approved full capture-exclusion implementation.

## Key References

- `docs/exec-plans/completed/036-interview-helper-live-session-full-product.md`
- `docs/TRACKS.md`
