...
Plans `007`, `010`, `011`, and `018` are now completed, and the queued roadmap is now centered on `012` guided setup plus profile copilot, then `013` source intelligence and faster discovery, `014` resume correctness and output quality, `015` staged apply automation, `016` shared agent auto compaction, and `017` browser substrate evaluation and direction. The main product sequence now starts from completed resume-import foundation `018`, while `016` remains a pull-forward infrastructure enabler and `017` remains a later benchmark-and-decision plan.
...
- Keep the queued follow-on roadmap centered on the product loop: completed `018` -> `012` -> `013` -> `014` -> `015`, with completed `011` treated as the shared-data baseline and `016` and `017` pulled forward only when they become the next concrete blocker.
...
- Start `012` next so guided setup can consume the new `018` import-run, candidate, and evidence substrate instead of only coarse resume warnings.
...
- Move `014` after that, but keep it downstream of `018` because resume quality cannot rely on imported proof, narrative, or answer-bank data until the extraction substrate is trustworthy.
