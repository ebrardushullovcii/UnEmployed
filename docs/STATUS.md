...
- Future discovery/browser work should start from the completed `017` evidence and stay source-generic
...
- Continue reducing real-app `Check source` cost when it blocks product use
- Clean persisted title/company quality when extracted jobs are otherwise useful
...
- Remove the `browser-runtime` to `browser-agent` catalog-session ownership seam when touching runtime ownership
