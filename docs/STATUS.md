...
- Discovery targets now have a source-debug bootstrap flow: Profile Preferences can launch a sequential debug run that learns bounded source guidance for discovery, detail navigation, and apply-entry paths, stores structured attempt/evidence artifacts, validates replayable instructions before promoting them, forwards validated guidance into the supported LinkedIn apply runtime, shows the learned instructions separately from the manual override field in Preferences, curates those instructions toward reusable site-specific guidance instead of raw phase boilerplate, keeps thin route-only findings in `draft` until the run proves stronger search/navigation plus detail/apply guidance, auto-opens managed LinkedIn sessions before the phase loop begins, and now treats internal agent/runtime failures as run failures instead of learned instructions.
