...
- Browser discovery can run either through the deterministic catalog seed or a configurable dedicated Chrome-profile browser agent, enabled by default on desktop when not explicitly disabled, backed by a user-authenticated local profile to reconcile earlier contradictory docs
