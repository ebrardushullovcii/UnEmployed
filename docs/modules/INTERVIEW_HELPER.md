...
- Microphone permission and desktop capture source readiness are Electron-backed in rehearsal. Browser speech recognition can feed microphone transcript segments where Chromium exposes it, but real raw microphone stream capture, meeting/system audio streams, local/platform STT, cloud fallback, and automatic meeting-native caption extraction still need live adapter implementation and hardware/user-permission validation.
