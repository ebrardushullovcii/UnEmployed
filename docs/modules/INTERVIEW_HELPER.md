...
- `packages/ai-providers` supplies deterministic Interview Helper providers for replayable development and test evidence, a configurable local-command STT provider when `UNEMPLOYED_INTERVIEW_LOCAL_STT_COMMAND` is set, plus OpenAI-compatible cue-card, screenshot-vision, and audio-transcription providers when `UNEMPLOYED_INTERVIEW_AI_API_KEY` or the shared `UNEMPLOYED_AI_API_KEY` is configured. Screenshot vision also honors `UNEMPLOYED_INTERVIEW_VISION_MODEL`, `UNEMPLOYED_AI_VISION_MODEL`, and the Job Finder resume-vision model fallback before using the shared text model. Model cue output is schema-validated, retries once on transient provider failure, and falls back to deterministic cue cards after retry exhaustion. Screenshot vision sends transient screenshot image data to the provider and retains only normalized observations, falling back to deterministic observations on provider failure. The Interview Helper service also converts invalid or thrown cue output into a quiet visible fallback card plus a provider diagnostic so a live session does not silently stall. Audio transcription uses transient local files or transient `/audio/transcriptions` requests and returns normalized transcript text only.
