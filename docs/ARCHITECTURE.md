...
- interview helper live session: Job Finder saved-job handoff or renderer setup/review UI -> typed preload -> Electron main-hosted `interview-helper` service -> `os-integration` adapters for protected overlays, audio, screenshots, hotkeys, and tray actions -> desktop-owned Electron microphone/media readiness checks, source-labeled transcript ingestion, optional browser speech bridge, temporary media stream probes, and `desktopCapturer` screenshot context with immediate temporary-file cleanup -> separate answer/transcript overlay BrowserWindows as consumers of service snapshots
...
- Interview Helper overlay capture exclusion is modeled as adapter-owned capability state. Electron `BrowserWindow.setContentProtection(true)` is requested by desktop overlay windows today, while real platform-specific verification and any native helper paths must stay behind `packages/os-integration`.
