...
describe('contracts', () => {
  ...
  test('parses a discovery run result and application attempt', () => {
    ...
    expect(discovery.agentMetadata?.debugFindings?.reliableControls[0]).toContain('Keyword search box')
  ...
  test('parses source-debug runs and instruction artifacts', () => {
    ...
    const artifact = SourceInstructionArtifactSchema.parse({
      ...
      applyGuidance: ['Prefer the inline apply button when the source exposes it.'],
      warnings: [],
      versionInfo: {
