...
describe('resume workspace quality helpers', () => {
  ...
  test('buildResumeCoverageComparison explains role, claim, order, and page changes', () => {
        ...
        entries: [
          {
            ...
            bullets: createBullets('experience_2_bullet', secondRole.achievements),
            included: false,
            sortOrder: 0,
          },
          {
    ...
    expect(
      comparison.roles.find(
        (role) => role.profileRecordId === 'experience_2',
      ),
    ).toMatchObject({
      status: 'hidden',
      included: false,
  ...
  test('assesses every visible generated claim against candidate-only evidence', () => {
    ...
    expect(validation.draftContentHash).toBe(buildResumeDraftContentHash(draft))
  })
