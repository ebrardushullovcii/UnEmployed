...
describe('createJobFinderWorkspaceService', () => {
  ...
  test('final source-instruction reviewer receives rich phase context and can override final curation', async () => {
    ...
    expect(reviewPrompts[0]).toContain('organize it into the final instruction artifact')
    expect(reviewPrompts[0]).toContain('full sequence of agent-led phase tests')
  ...
  test('verify source instructions replays the chosen artifact without overwriting it mid-run', async () => {
    ...
    seed.sourceInstructionArtifacts = [
      {
        ...
        searchGuidance: ['Use the existing validated keyword search box to change the result set.'],
        detailGuidance: ['Use same-host detail pages as the canonical source of job data.'],
        applyGuidance: ['Use the on-site apply entry when the detail page exposes it.'],
    ...
    const instrumentedRuntime: BrowserSessionRuntime = {
      ...browserRuntime,
      runAgentDiscovery(source, options) {
        replayInstructionsByLabel.set(options.siteLabel, [...(options.siteInstructions ?? [])])
    ...
    const snapshot = await workspaceService.verifySourceInstructions('target_linkedin_default', 'instruction_existing_validated')
    const artifacts = await repository.listSourceInstructionArtifacts()
    const originalArtifact = artifacts.find((artifact) => artifact.id === 'instruction_existing_validated')
    const successorArtifact = artifacts.find(
      (artifact) => artifact.id !== 'instruction_existing_validated' && artifact.targetId === 'target_linkedin_default'
    )
    ...
    expect(replayInstructionsByLabel.get('Primary target Replay Verification')?.join('\n')).not.toContain(
      'Keep this source in draft if replay fails later.'
    )
    expect(originalArtifact?.status).toBe('validated')
    expect(originalArtifact?.navigationGuidance).toEqual(['Use the existing validated collection route first.'])
    expect(successorArtifact?.status).toBe('validated')
    expect(successorArtifact?.verification?.outcome).toBe('passed')
    expect(snapshot.searchPreferences.discovery.targets[0]?.validatedInstructionId).toBe(successorArtifact?.id ?? null)
  })
