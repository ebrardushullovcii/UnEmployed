...
function toEditableSourceInstructionArtifactInput(artifact: {
  id: string;
  notes: string | null;
...
function createResumeExtraction(
...
function createStrongSourceDebugFindingsByPhase(): SourceDebugPhaseMap<AgentDebugFindings | null> {
  return {
    access_auth_probe: {
      summary:
...
function createThinSourceDebugFindingsByPhase(): SourceDebugPhaseMap<AgentDebugFindings | null> {
  return {
    job_detail_validation: {
      summary:
...
function createUnprovenVisibleControlFindingsByPhase(): SourceDebugPhaseMap<AgentDebugFindings | null> {
  return {
    site_structure_mapping: {
...
function createUrlShortcutOnlyFindingsByPhase(): SourceDebugPhaseMap<AgentDebugFindings | null> {
  return {
    site_structure_mapping: {
      summary:
...
function createMixedAuthSurfaceFindingsByPhase(): SourceDebugPhaseMap<AgentDebugFindings | null> {
  return {
    access_auth_probe: {
      summary:
...
function createNoisySourceDebugFindingsByPhase(): SourceDebugPhaseMap<AgentDebugFindings | null> {
  return {
    site_structure_mapping: {
...
function createExtractionAiClient(
...
describe("createJobFinderWorkspaceService", () => {
  ...
  test("agent discovery streams activity and keeps discovery-only jobs pending", async () => {
    ...
    const catalog = createBrowserRuntime().runDiscovery(
      "target_site",
  ...
  test("agent discovery uses the active draft-or-validated instructions for the matching target", async () => {
    ...
    const catalog = await createBrowserRuntime().runDiscovery(
      "target_site",
      createSeed().searchPreferences,
    );
  ...
  test("saveSourceInstructionArtifact updates a bound target artifact and rejects unbound edits", async () => {
    ...
    const updatedSnapshot =
      await workspaceService.saveSourceInstructionArtifact(
    ...
    await expect(
      workspaceService.saveSourceInstructionArtifact(
        "target_linkedin_default",
        {
          ...toEditableSourceInstructionArtifactInput(originalArtifact),
  ...
  test("agent discovery abort keeps streamed activity and avoids persistence", async () => {
    ...
    const discoveryResult = await createBrowserRuntime().runDiscovery(
      "target_site",
      createSeed().searchPreferences,
    );
  ...
  test("extracts profile details from stored resume text", async () => {
    ...
    expect(
      snapshot.profile.skillGroups.highlightedSkills.length,
    ).toBeGreaterThan(0);
  ...
  test("verify source instructions replays the chosen artifact without overwriting it mid-run", async () => {
      ...
      [
        {
          ...
          discoveryMethod: "catalog_seed",
          canonicalUrl:
            "https://www.linkedin.com/jobs/view/linkedin_verify_case",
          title: "Frontend Engineer",
          company: "Signal Systems",
          location: "Remote",
          workMode: ["remote"],
    ...
    const instrumentedRuntime: BrowserSessionRuntime = {
      ...
      runAgentDiscovery(source, options) {
        ...
        return browserRuntime.runAgentDiscovery!(source, options);
      },
    };
    const workspaceService = createJobFinderWorkspaceService({
      repository,
    ...
    expect(
      replayInstructionsByLabel.get("Primary target Replay Verification"),
    ).toEqual(
      expect.arrayContaining([
        "Use the existing validated collection route first.",
        "Use the existing validated keyword search box to change the result set.",
    ...
    expect(
      snapshot.searchPreferences.discovery.targets[0]?.validatedInstructionId,
    ).toBe(successorArtifact?.id ?? null);
  });
