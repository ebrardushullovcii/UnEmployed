...
const LEGACY_DEFAULT_TARGET_STARTING_URL =
...
function parseSourceInstructionReviewOverride(
  raw: unknown,
...
function getActiveDiscoveryTargets(
  searchPreferences: JobSearchPreferences,
): JobDiscoveryTarget[] {
...
function getPreferredSessionAdapter(
  searchPreferences: JobSearchPreferences,
): JobSource {
...
function lineMentionsAnyKeyword(
  value: string,
...
function isInternalSourceDebugFailure(
...
function filterSourceDebugWarnings(
...
function summarizeCanonicalUrlBehavior(
...
function warningSuggestsAuthRestriction(
  value: string | null | undefined,
): boolean {
  const normalized = normalizeText(value ?? "");
...
function collectAttemptInstructionGuidance(
...
function deriveGuidanceFromPhaseEvidence(
  attempt: SourceDebugWorkerAttempt | undefined,
): string[] {
  ...
  const hasLocationCombobox = controls.some(
    (control) =>
      /\bcombobox\b/i.test(control) && /\b(location|qyteti)\b/i.test(control),
  );
  ...
  const attemptedLocationControl = attemptedControls.some((value) =>
    /\b(location|qyteti)\b/i.test(value),
  );
  const attemptedIndustryControl = attemptedControls.some((value) =>
    /\b(industry|industria|category|department)\b/i.test(value),
  );
...
function lineContradictsVisiblePhaseEvidence(
  value: string,
...
function isSupersededByStrongerSourceInstructionGuidance(
  value: string,
  family: string,
): boolean {
  const normalized = normalizeText(value);
...
function reconcileFinalSourceInstructionGuidance(input: {
  ...
  return {
    navigationGuidance: keptItems
      .filter((item) => item.section === "navigation")
      .map((item) => item.line),
    searchGuidance: keptItems
      .filter((item) => item.section === "search")
      .map((item) => item.line),
    detailGuidance: keptItems
      .filter((item) => item.section === "detail")
      .map((item) => item.line),
    applyGuidance: keptItems
...
function toDiscoverySessionState(
...
function nextJobStatusFromAttempt(
  job: SavedJob,
...
export function createJobFinderWorkspaceService(
  ...
  function buildInstructionGuidance(
  ...
  async function persistSourceDebugRun(
    run: SourceDebugRunRecord,
  ): Promise<void> {
  ...
  async function persistBrowserSessionState(
  ...
  function buildSourceDebugPhaseSummary(
  ...
  function synthesizeSourceInstructionArtifact(
    ...
    const hasSearchGuidanceWithoutPositiveProof =
      searchGuidance.length > 0 && !hasPositiveReusableSearchGuidance;
  ...
  async function getWorkspaceSnapshot(): Promise<JobFinderWorkspaceSnapshot> {
    ...
    const discoverySessions = await refreshDiscoverySessions(
    ...
    try {
      ...
      >({
        ...
        executePhase: async (phase, index) => {
          ...
          const debugResult = await browserRuntime.runAgentDiscovery?.(
            adapterKind,
            {
              userProfile: profile,
              searchPreferences: {
          ...
          const debugFindings =
            debugResult.agentMetadata?.debugFindings ?? null;
          ...
          const artifact = SourceDebugWorkerAttemptSchema.parse({
            ...
            strategyLabel:
              phasePacket.strategyLabel ?? formatStatusLabel(phase),
            strategyFingerprint,
            confirmedFacts,
          ...
          return {
            artifact,
            stop:
              outcome === "blocked_auth" || outcome === "blocked_manual_step",
          };
        },
        afterPhase: async (phase, _index, attempt) => {
          ...
          if (
            ...
            await repository.upsertSourceDebugRun(run);
            await persistSourceDebugRun(run);
            await saveDiscoveryTargetUpdate(
              normalizedTarget.id,
              (currentTarget) => ({
                ...currentTarget,
      ...
      run = SourceDebugRunRecordSchema.parse({
        ...
        activePhase: null,
        finalSummary:
          verification.proofSummary ?? "Source debug workflow completed.",
  ...
  return {
    ...
    async openBrowserSession() {
      ...
      const session = await browserRuntime.openSession(
    ...
    async checkBrowserSession() {
      ...
      const session = await browserRuntime.getSessionState(
        getPreferredSessionAdapter(searchPreferences),
      );
      await persistBrowserSessionState(session);
      return getWorkspaceSnapshot();
    ...
    async runDiscovery() {
      ...
      try {
        await openRunBrowserSession(adapterKind);
        browserSessionOpened = true;
        discoveryResult = await browserRuntime.runDiscovery(
          adapterKind,
          enrichedPreferences,
        );
      } finally {
      ...
      try {
        for (const [index, target] of targets.entries()) {
          ...
          try {
            ...
            const customInstructions = splitCustomDiscoveryInstructions(
            ...
            const discoveryResult = await browserRuntime.runAgentDiscovery(
              adapterKind,
              {
                userProfile: profile,
                searchPreferences: {
                  targetRoles:
                    enrichedPreferences.targetRoles.length > 0
                      ? enrichedPreferences.targetRoles
                      : [DEFAULT_ROLE],
            ...
            const persistedWorkingJobIds = new Set(
            ...
            if (settings.discoveryOnly) {
              const nextPendingJobs = mergeResult.mergedJobs.filter(
                (job) => !persistedWorkingJobIds.has(job.id),
              );
              for (const job of mergeResult.mergedJobs.filter((job) =>
                persistedWorkingJobIds.has(job.id),
              )) {
              ...
              workingSavedJobs = mergeResult.mergedJobs.filter(
                (job) =>
                  persistedWorkingJobIds.has(job.id) && !newJobIds.has(job.id),
              );
            ...
            activeRun = updateTargetExecution(
              ...
              (execution) => ({
                ...
                jobsPersisted: settings.discoveryOnly
                  ? 0
                  : mergeResult.newJobs.length,
                jobsStaged: settings.discoveryOnly
                  ? mergeResult.newJobs.length
                  : 0,
            ...
            activeRun = DiscoveryRunRecordSchema.parse({
              ...
              summary: {
                ...
                jobsPersisted:
                  activeRun.summary.jobsPersisted +
                  (settings.discoveryOnly ? 0 : mergeResult.newJobs.length),
                jobsStaged:
                  activeRun.summary.jobsStaged +
                  (settings.discoveryOnly ? mergeResult.newJobs.length : 0),
                duplicatesMerged:
        ...
        const runOutcome: DiscoveryRunState =
          failedTargets > 0 ? "failed" : "completed";
        activeRun = DiscoveryRunRecordSchema.parse({
          ...activeRun,
          state: runOutcome,
          completedAt: new Date().toISOString(),
        ...
        emitActivity(
          createDiscoveryEvent({
            ...
            resolvedAdapterKind: null,
            message: aborted
              ? "Discovery run cancelled before completion"
              : `Discovery run failed: ${error instanceof Error ? error.message : "Unknown error"}`,
            url: null,
            jobsFound: null,
            jobsPersisted: null,
            jobsStaged: null,
    ...
    async cancelSourceDebug(runId) {
      if (
        activeSourceDebugExecutionId !== runId ||
        !activeSourceDebugAbortController
      ) {
        return getWorkspaceSnapshot();
      }
    ...
    async getSourceDebugRun(runId) {
      const run = (await repository.listSourceDebugRuns()).find(
    ...
    async acceptSourceInstructionDraft(targetId, instructionId) {
      const artifact = (await repository.listSourceInstructionArtifacts()).find(
    ...
    async generateResume(jobId) {
      ...
      const contentText =
        draft.fullText ||
        buildTailoredResumeText(profile, job, previewSections);
    ...
    async approveApply(jobId) {
      ...
      const executionResult = await browserRuntime.executeEasyApply(
        job.source,
        {
          job,
          asset,
