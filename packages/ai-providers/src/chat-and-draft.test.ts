...
describe('openai-compatible chat and draft behavior', () => {
  ...
  test('fills missing structured draft fields with deterministic fallback content', async () => {
    ...
    try {
      ...
      expect(result.experienceEntries).toEqual(deterministicFallback.experienceEntries)
      expect(result.educationEntries).toEqual(deterministicFallback.educationEntries)
      expect(result.additionalSkills).toEqual(deterministicFallback.additionalSkills)
  ...
  test('compacts oversized resume assistant payloads before sending them to the model', async () => {
    ...
    try {
      ...
      await client.reviseResumeDraft({
        ...
        validationIssues: Array.from({ length: 20 }, (_, index) => `Validation issue ${index + 1} ${'detail '.repeat(80)}`),
        researchContext: {
          companyNotes: Array.from({ length: 20 }, (_, index) => `Company note ${index + 1} ${'context '.repeat(80)}`),
          domainVocabulary: Array.from({ length: 20 }, (_, index) => `Vocabulary ${index + 1} ${'term '.repeat(40)}`),
          priorityThemes: Array.from({ length: 20 }, (_, index) => `Theme ${index + 1} ${'priority '.repeat(40)}`),
        },
      })
      ...
      expect(userPayload.request?.length ?? 0).toBeLessThan(oversizedRequest.length)
  ...
  test('compacts oversized draft-creation payloads before sending them to the model', async () => {
    const fetchMock = mockCapturingJsonFetch({
      choices: [
        {
          message: {
            content: JSON.stringify({ label: 'Tailored Resume', summary: 'Short summary', experienceHighlights: ['Highlight'] })
          }
        }
      ]
    })
    ...
    try {
      ...
      await client.createResumeDraft({
        ...
        job: {
          ...
          responsibilities: Array.from({ length: 20 }, (_, index) => `Responsibility ${index + 1} ${'detail '.repeat(80)}`),
          minimumQualifications: Array.from({ length: 20 }, (_, index) => `Minimum qualification ${index + 1} ${'detail '.repeat(80)}`),
          preferredQualifications: Array.from({ length: 20 }, (_, index) => `Preferred qualification ${index + 1} ${'detail '.repeat(80)}`),
        },
        resumeText: `Resume text ${'history '.repeat(8000)}`,
        evidence: {
          summary: Array.from({ length: 20 }, (_, index) => `Summary evidence ${index + 1} ${'detail '.repeat(80)}`),
          candidateSummary: Array.from({ length: 20 }, (_, index) => `Candidate summary ${index + 1} ${'detail '.repeat(80)}`),
          experience: Array.from({ length: 20 }, (_, index) => `Experience evidence ${index + 1} ${'detail '.repeat(80)}`),
          skills: Array.from({ length: 20 }, (_, index) => `Skill ${index + 1} ${'detail '.repeat(40)}`),
          keywords: Array.from({ length: 20 }, (_, index) => `Keyword ${index + 1} ${'detail '.repeat(40)}`),
        },
        researchContext: {
          companyNotes: Array.from({ length: 20 }, (_, index) => `Company note ${index + 1} ${'detail '.repeat(80)}`),
          domainVocabulary: Array.from({ length: 20 }, (_, index) => `Vocabulary ${index + 1} ${'detail '.repeat(40)}`),
          priorityThemes: Array.from({ length: 20 }, (_, index) => `Theme ${index + 1} ${'detail '.repeat(40)}`),
        },
      })
      ...
      expect(userPayload.resumeText?.length ?? 0).toBeLessThan(`Resume text ${'history '.repeat(8000)}`.length)
      expect(userPayload.resumeText).toContain('[truncated')
      expect(Array.isArray(userPayload.evidence?.summary)).toBe(true)
      expect(Array.isArray(userPayload.researchContext?.companyNotes)).toBe(true)
      expect(userPayload.job?.description?.length ?? 0).toBeLessThan(`Job description ${'requirement '.repeat(5000)}`.length)
    } finally {
      fetchMock.restore()
    }
  })
  ...
  test('compacts oversized profile copilot payloads before sending them to the model', async () => {
    ...
    try {
      ...
      await client.reviseCandidateProfile({
        ...
        searchPreferences: {
          ...createPreferences(),
          targetRoles: Array.from({ length: 20 }, (_, index) => `Role ${index + 1} ${'detail '.repeat(40)}`),
          locations: Array.from({ length: 20 }, (_, index) => `Location ${index + 1} ${'detail '.repeat(40)}`),
        },
        ...
        request,
        conversationFacts: Array.from({ length: 20 }, (_, index) => `Fact ${index + 1} ${'detail '.repeat(80)}`),
      })
      ...
      expect(userPayload.request).toContain('change my experience to only 5 years')
  ...
  test('compacts 200k-token-class profile copilot payloads before provider submission', async () => {
    ...
    try {
      ...
      const relevantReviewItems: ProfileCopilotRelevantReviewItem[] = Array.from({ length: 120 }, (_, index) => ({
      ...
      expect(compactedPayload.request).toContain('change my experience to only 5 years')
