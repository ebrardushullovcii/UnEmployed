...
const skillSectionAliases = [
  "SKILLS",
  "TECHNICAL SKILLS",
  "CORE SKILLS",
  "KEY SKILLS",
] as const;
const summarySectionAliases = [
  "ABOUT MYSELF",
  "ABOUT",
  "SUMMARY",
  "PROFILE",
...
const skillCategoryHeadingPattern =
...
function findSectionBodyLinesByAliases(
  lines: readonly string[],
...
function inferPortfolioUrl(resumeText: string): string | null {
  return (
    ...
    ).find((url) => !/linkedin\.com/i.test(url)) ?? null
  );
}
...
function inferSkills(
  ...
  const matchedKnownSkills = uniqueStrings(
    knownSkillPhrases.filter((skill) =>
      sectionText.toLowerCase().includes(skill.toLowerCase()),
    ),
  );
...
const likelyPersonalWebsitePaths = new Set([
  "",
  "/",
  "/about",
  "/contact",
...
function inferPersonalWebsiteUrl(resumeText: string): string | null {
  return (
    extractAllUrls(resumeText).find((url) => isLikelyPersonalWebsiteUrl(url)) ??
    null
  );
}
...
function inferKnownPhrases(text: string, phrases: readonly string[]): string[] {
  ...
  return uniqueStrings(
    phrases.filter((phrase) => lowerText.includes(phrase.toLowerCase())),
  );
}
...
function inferSalaryCurrencyFromLocation(
  ...
  const knownMappings: Array<[RegExp, string]> = [
    [
      /prishtina|kosovo|germany|berlin|france|paris|spain|italy|netherlands|belgium|austria|portugal|finland|ireland|greece/,
      "EUR",
    ],
...
function inferProfessionalSummary(
  summary: string | null,
  headline: string | null,
  skills: readonly string[],
) {
  const firstSentence =
    cleanLine(summary?.split(/(?<=[.!?])\s+/)[0] ?? "") || null;
...
function splitExperienceBlocks(lines: readonly string[]): string[][] {
  ...
  for (const line of lines) {
    ...
    if (startsNewBlock) {
      ...
      currentBlock = pendingCompanyMarker
        ? [pendingCompanyMarker, line]
        : [line];
...
function inferExperienceEntries(resumeText: string) {
  ...
  return splitExperienceBlocks(sectionLines)
    .map((block) => {
      ...
      const titleValue =
        cleanLine(
          headerLine.replace(dateRangePattern, "").replace(/[|,–—-]+\s*$/g, ""),
        ) || null;
      ...
      const summaryLine =
        detailLines.find((line) => !/^project lead\b/i.test(line)) ?? null;
      const achievementLines = detailLines.filter(
...
function inferEducationEntries(resumeText: string) {
  ...
  const schoolKeywordMatch = educationLine.match(
    /\b(College|University|School|Institute|Kolegji)\b/i,
  );
  const schoolName =
    schoolKeywordMatch?.index !== undefined
      ? cleanLine(educationLine.slice(schoolKeywordMatch.index))
      : null;
  const detailsPart =
    schoolKeywordMatch?.index !== undefined
      ? cleanLine(educationLine.slice(0, schoolKeywordMatch.index))
      : educationLine;
...
function buildProfileExtractionNotes(input: {
  ...
  if (!input.fullName) {
    notes.push(
      "Review the imported name because the parser could not confidently extract it.",
    );
  }

  if (!input.headline) {
    notes.push(
      "Add a preferred headline if the resume does not expose a clear current role.",
    );
  }

  if (!input.summary) {
    notes.push(
      "Add a short professional summary if the resume does not include one.",
    );
  }

  if (!input.currentLocation) {
    notes.push(
      "Confirm the preferred location because the resume did not expose a clear location line.",
    );
  }
...
function buildDeterministicResumeProfileExtraction(
  ...
  const personalWebsiteUrl =
    inferPersonalWebsiteUrl(input.resumeText) ??
    input.existingProfile.personalWebsiteUrl;
  const portfolioUrl =
    inferPortfolioUrl(input.resumeText) ??
    personalWebsiteUrl ??
    input.existingProfile.portfolioUrl;
  ...
  return ResumeProfileExtractionSchema.parse({
    ...
    timeZone:
      inferTimeZoneFromLocation(currentLocation) ??
      input.existingProfile.timeZone,
    salaryCurrency:
    ...
    githubUrl:
      inferGithubUrl(input.resumeText) ?? input.existingProfile.githubUrl,
    personalWebsiteUrl,
    ...
    preferredLocations: inferLocations(
      currentLocation,
...
function mergeExperienceExtractionEntries(
  ...
  const merged = primary.map((entry) => {
    let matchIndex = fallback.findIndex((fb, idx) => {
      ...
      const titleMatch =
        (entry.title &&
          fb.title &&
          entry.title.toLowerCase() === fb.title.toLowerCase()) ||
    ...
    if (matchIndex === -1) {
      matchIndex = fallback.findIndex(
        (fb, idx) => !matchedFallbackIndices.has(idx),
      );
    }
  ...
  const unmatchedFallback = fallback.filter(
    (_, idx) => !matchedFallbackIndices.has(idx),
  );
...
function mergeEducationExtractionEntries(
  ...
  const merged = primary.map((entry) => {
    ...
    let matchIndex = fallback.findIndex((fb, idx) => {
      if (matchedFallbackIndices.has(idx)) return false;
      return (
        normalize(entry.schoolName) === normalize(fb.schoolName) &&
...
function mergeLinkExtractionEntries(
  ...
  const fallbackByUrl = new Map(
    fallback.map((entry) => [entry.url ?? "", entry]),
  );
  ...
  const unmatchedFallback = fallback.filter(
    (entry) => !primaryUrls.has(entry.url ?? ""),
  );

  return [...merged, ...unmatchedFallback];
}

function scoreExperienceEntries(
  entries: readonly ResumeProfileExtraction["experiences"][number][],
): number {
  if (entries.length === 0) {
    return 0;
  }
...
function scoreEducationEntries(
  entries: readonly ResumeProfileExtraction["education"][number][],
): number {
  if (entries.length === 0) {
    return 0;
  }
  ...
  const total = entries.reduce((score, entry) => {
    return (
      score +
      (entry.schoolName ? 2 : 0) +
      (entry.degree ? 1 : 0) +
      (entry.fieldOfStudy ? 1 : 0)
    );
  }, 0);
...
function choosePreferredHeadline(
  primary: string | null,
...
function completeResumeExtraction(
  ...
  const useFallbackExperiences =
    scoreExperienceEntries(primary.experiences) <
    scoreExperienceEntries(fallback.experiences);
  const useFallbackEducation =
    scoreEducationEntries(primary.education) <
    scoreEducationEntries(fallback.education);
  ...
  return ResumeProfileExtractionSchema.parse({
    ...
    personalWebsiteUrl:
      primary.personalWebsiteUrl ?? fallback.personalWebsiteUrl,
    professionalSummary: {
      shortValueProposition:
        primary.professionalSummary.shortValueProposition ??
        fallback.professionalSummary.shortValueProposition,
      fullSummary:
        primary.professionalSummary.fullSummary ??
        fallback.professionalSummary.fullSummary,
      careerThemes:
      ...
      leadershipSummary:
        primary.professionalSummary.leadershipSummary ??
        fallback.professionalSummary.leadershipSummary,
      domainFocusSummary:
        primary.professionalSummary.domainFocusSummary ??
        fallback.professionalSummary.domainFocusSummary,
      strengths:
    ...
    skillGroups: {
      coreSkills:
        primary.skillGroups.coreSkills.length > 0
          ? primary.skillGroups.coreSkills
          : fallback.skillGroups.coreSkills,
      tools:
        primary.skillGroups.tools.length > 0
          ? primary.skillGroups.tools
          : fallback.skillGroups.tools,
      ...
      softSkills:
        primary.skillGroups.softSkills.length > 0
          ? primary.skillGroups.softSkills
          : fallback.skillGroups.softSkills,
      highlightedSkills:
    ...
    targetRoles:
      primary.targetRoles.length > 0
        ? primary.targetRoles
        : fallback.targetRoles,
    preferredLocations:
      primary.preferredLocations.length > 0
        ? primary.preferredLocations
        : fallback.preferredLocations,
    experiences: useFallbackExperiences
      ? fallback.experiences
      : mergedExperiences,
    ...
    projects:
      primary.projects.length > 0 ? primary.projects : fallback.projects,
    spokenLanguages:
      primary.spokenLanguages.length > 0
        ? primary.spokenLanguages
        : fallback.spokenLanguages,
...
export function createDeterministicJobFinderAiClient(
  ...
  return {
    ...
    tailorResume(input) {
      ...
      return Promise.resolve(
        TailoredResumeDraftSchema.parse({
