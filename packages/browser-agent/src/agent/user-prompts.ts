...
export function buildForcedFinishPrompt(
  state: AgentState,
  config: AgentConfig,
): string {
...
${config.searchPreferences.locations.length === 0 ? "Do not infer or add the candidate's current or resume location as a search constraint.\n" : ""}
