...
const SEEDED_QUERY_GUARD_TOOLS = new Set([
  "navigate",
  "click",
  "fill",
  "select_option",
...
const SEEDED_QUERY_GUARD_LOCATION_NOISE = new Set([
  "remote",
  "hybrid",
  "on",
  "site",
...
function isSeededSearchSurfaceUrl(value: string, seedUrl?: string): boolean {
  try {
    ...
    return (
      url.hostname.toLowerCase() === seededUrl.hostname.toLowerCase() &&
...
function tokenizeSeededQueryValue(
  ...
  return [
    ...new Set(
      tokens.filter((token) => !SEEDED_QUERY_GUARD_LOCATION_NOISE.has(token)),
    ),
  ];
}

function parseSearchQueryUrl(
  value: string,
  seedUrl?: string,
): {
  hostname: string;
  ...
  try {
    const url = new URL(value);
    const pathname = url.pathname.toLowerCase();
    const { ignoredParams, locationParams } = getSeededQueryRuleParams(
      url.hostname,
    );
    ...
    for (const [key, rawValue] of url.searchParams.entries()) {
      ...
      const tokens = tokenizeSeededQueryValue(
        trimmedValue,
        key,
        locationParams,
      );
...
function normalizeRestoreCandidateUrl(
...
function queryDropsSeededTokens(
  candidateTokens: readonly string[],
  seededTokens: readonly string[],
): boolean {
...
export function detectSeededSearchQueryDrift(input: {
  ...
  const hasCandidateEvidence =
    input.state.collectedJobs.length > 0 ||
    input.state.deferredSearchExtractions.size > 0;
  const leftSeededSearchSurfaceEarly =
    !hasCandidateEvidence &&
    isSeededSearchSurfaceUrl(
      input.previousUrl ?? seededQuery.seedUrl,
      seededQuery.seedUrl,
    ) &&
  ...
  const dropsSeededQuery = [...seededQuery.paramTokens.entries()].some(
    ([key, seededTokens]) =>
      queryDropsSeededTokens(
        candidateQuery.paramTokens.get(key) ?? [],
        seededTokens,
      ),
  );
...
function syncCurrentUrlTracking(input: {
  ...
  const guardViolation = detectSeededSearchQueryDrift({
    ...
    state: input.state,
    url: nextUrl,
    ...(input.state.lastStableUrl
      ? { previousUrl: input.state.lastStableUrl }
      : {}),
  });
...
export async function restoreSeededQuerySurfaceIfNeeded(input: {
  ...
  const seededQuery = parseSeededSearchQuery(input.config);
  const previousUrl =
    input.state.lastStableUrl || seededQuery?.seedUrl || currentUrl;
...
function createDeferredSearchExtraction(input: {
  pageUrl: string;
  pageText: string;
  structuredDataCandidates?: Parameters<
    typeof buildStructuredCandidateJobs
  >[0]["structuredDataCandidates"];
  cardCandidates?: Parameters<
    typeof buildStructuredCandidateJobs
  >[0]["cardCandidates"];
}): DeferredSearchExtraction {
  return {
    ...
    structuredDataCandidates: input.structuredDataCandidates
      ? [...input.structuredDataCandidates]
      : [],
    cardCandidates: input.cardCandidates ? [...input.cardCandidates] : [],
  };
}

function getStructuredCandidateKey(
  candidate: Parameters<
    typeof buildStructuredCandidateJobs
  >[0]["structuredDataCandidates"] extends readonly (infer T)[] | undefined
    ? T
    : never,
): string {
  const canonicalUrl =
...
function getCardCandidateKey(
  ...
  candidate: Parameters<
    typeof buildStructuredCandidateJobs
  >[0]["cardCandidates"] extends readonly (infer T)[] | undefined
    ? T
    : never,
): string {
...
function mergeCandidateList<T>(
  ...
  const getRichnessScore = (candidate: T): number => {
    ...
    return Object.values(candidate as Record<string, unknown>).reduce<number>(
      (score, value) => {
...
function mergeDeferredSearchExtraction(
  ...
  const mergedCardCandidates = mergeCandidateList(
    ...
    (candidate) =>
      getCardCandidateKey(existing.pageUrl || incoming.pageUrl, candidate),
  );
  const shouldPreferIncomingPageText =
    incoming.pageText.length >= existing.pageText.length ||
    mergedStructuredCandidates.length >
      (existing.structuredDataCandidates?.length ?? 0) ||
...
export async function executeToolCall(
  ...
  try {
    ...
    console.error(
      `[Agent] Failed to parse tool arguments for ${toolName}:`,
      parseError,
    );
    return {
      success: false,
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const normalizedPageType =
        extractData.pageType === "job_detail" ? "job_detail" : "search_results";
      const remainingJobs = Math.max(
        0,
        config.targetJobCount - state.collectedJobs.length,
      );
      const expandedSearchResultsBudget =
        getSearchResultsExtractionReviewBudget(config);
      // Search-result extraction intentionally uses the expanded review cap when
      // normalizedPageType === 'search_results', so effectiveMaxJobs ignores args.maxJobs/requestedMaxJobs
      // and keeps the larger search-surface review budget without scaling to the full target count.
      const maxJobs =
      ...
      const structuredDataCandidates = Array.isArray(
        ...
        ? (extractData.structuredDataCandidates as NonNullable<
            Parameters<
              typeof buildStructuredCandidateJobs
            >[0]["structuredDataCandidates"]
          >)
      ...
      const cardCandidates = Array.isArray(extractData.cardCandidates)
        ? (extractData.cardCandidates as NonNullable<
            Parameters<typeof buildStructuredCandidateJobs>[0]["cardCandidates"]
          >)
        : [];
      const fastPathJobs =
        normalizedPageType === "search_results"
      ...
      if (shouldDeferSearchExtraction) {
        ...
        const existingSnapshot = state.deferredSearchExtractions.get(
      ...
      if (addedCount > 0) {
        console.log(
          `[Agent] +${addedCount} jobs (${state.collectedJobs.length} total) from ${extractData.pageUrl.slice(0, 60)}...`,
        );
      }
        ...
        try {
          ...
          const recoveryError =
            resolveError instanceof Error
              ...
              : new Error("Failed to recover from a closed page error.", {
                  cause: error,
                });
          return {
            success: false,
  ...
  throw new Error(
    `Unreachable: executeToolCall exhausted retries without returning for ${toolName}`,
  );
}
