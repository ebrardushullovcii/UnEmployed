...
import {
  ...
  buildLooseAccessibleNamePattern,
  ClickSchema,
  FillSchema,
  SelectOptionSchema,
  resolveRoleLocator,
...
describe("interactive tool schemas", () => {
  test("reject invalid aria roles at the schema boundary", () => {
    expect(
      ClickSchema.safeParse({ role: "dialog", name: "Open", index: 0 }).success,
    ).toBe(false);
    expect(
      FillSchema.safeParse({ role: "grid", name: "Search", text: "engineer" })
        .success,
    ).toBe(false);
    expect(
      FillSchema.safeParse({ role: "link", name: "Search", text: "engineer" })
        .success,
    ).toBe(false);
    expect(
  ...
  test("rejects whitespace-only strings at the schema boundary", () => {
    expect(
      ClickSchema.safeParse({ role: "button", name: "   ", index: 0 }).success,
    ).toBe(false);
    expect(
      FillSchema.safeParse({ role: "textbox", name: " Search ", text: "   " })
        .success,
    ).toBe(false);
    expect(
      SelectOptionSchema.safeParse({
...
describe("resolveRoleLocator", () => {
  test("accepts LinkedIn-style button naming variants like All filters versus Show all filters", async () => {
    ...
    const page = {
      getByRole: vi.fn((_role: string, options?: { exact?: boolean; name?: string }) => {
  ...
  test("accepts job link naming variants that append Verified job", async () => {
    ...
    const page = {
      getByRole: vi.fn((_role: string, options?: { exact?: boolean; name?: string }) => {
        if (
          options?.exact &&
          options.name === "Senior Next.js Developer (Verified job)"
        ) {
    ...
    const locator = await resolveRoleLocator(
      page,
      "link",
