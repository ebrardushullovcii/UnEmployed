...
describe("click", () => {
  test("fails fast when no matching role and name are present", async () => {
    ...
    const result = await getInteractionTool("click").execute(
      {
        role: "link",
  ...
  test("blocks repeated disallowed-url click failures after the threshold is reached", async () => {
    ...
    expect(result).toEqual({
      success: false,
      error:
        'Skipping repeated click attempt for link "Customer Experience Specialist" after 2 similar failures: Navigation went to disallowed URL: https://malicious.example.net/phish',
      data: expect.objectContaining({
        role: "link",
        name: "Customer Experience Specialist",
        index: 0,
  ...
  test("falls back to clicking the associated checkbox label when the input intercepts pointer events", async () => {
    ...
    const checkboxLocator = {
      ...
      click: vi
        ...
        .mockRejectedValue(
          new Error(
            'locator.click: Timeout 5000ms exceeded. <label for="advanced-filter-workplaceType-2"> intercepts pointer events',
          ),
        ),
      locator: vi.fn((selector: string) =>
        selector.startsWith("xpath=ancestor::label")
          ? {
              first: () => ({
                count: vi.fn().mockResolvedValue(0),
                isVisible: vi.fn().mockResolvedValue(false),
  ...
  test("blocks repeated identical click failures after the threshold is reached", async () => {
    ...
    expect(result).toEqual({
      success: false,
      error:
        'Skipping repeated click attempt for link "Senior Next.js Developer" after 2 similar failures: No link matched accessible name "Senior Next.js Developer".',
      data: expect.objectContaining({
        role: "link",
        name: "Senior Next.js Developer",
        index: 0,
  ...
  test("does not block button attempts from a prior link failure key", async () => {
    ...
    expect(result).toEqual({
      success: false,
      error:
        'No button matched accessible name "Senior Next.js Developer (Verified job) Proxify Kosovo Remote".',
      data: expect.objectContaining({
        role: "button",
        index: 0,
        errorType: "click_failed",
  ...
  test("blocks repeated pointer-interception click failures after the threshold is reached", async () => {
        ...
        failedInteractionAttempts: new Map([
          [
            "click::checkbox::filter by remote::0",
            {
              count: 2,
              lastError:
                "The matched control is present, but another visible element intercepts direct pointer clicks.",
            },
          ],
        ]),
      }),
    ...
    expect(result).toEqual({
      success: false,
      error:
        'Skipping repeated click attempt for checkbox "Filter by Remote" after 2 similar failures: The matched control is present, but another visible element intercepts direct pointer clicks.',
      data: expect.objectContaining({
        role: "checkbox",
        index: 0,
        errorType: "repeated_click_blocked",
