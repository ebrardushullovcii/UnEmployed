...
function createPage(): Pick<Page, 'goto' | 'waitForTimeout' | 'url' | 'title' | 'locator' | 'evaluate'> {
...
describe('runAgentDiscovery', () => {
  ...
  test('compaction preserves tool-call context so later llm requests do not start with orphan tool messages', async () => {
    const page = createPage() as Page
    const llmCalls: ToolCall[][] = [
      [createToolCall('navigate', { url: 'https://www.linkedin.com/jobs/search/' }, 'tool_nav_1')],
      [createToolCall('navigate', { url: 'https://www.linkedin.com/jobs/search/' }, 'tool_nav_2')],
      [createToolCall('finish', {
        reason: 'Enough evidence collected.',
        summary: 'Recommendation route stayed reachable after compaction.',
        reliableControls: ['Show all top job picks route stayed available after compaction'],
        trickyFilters: [],
          ...
          const hasMatchingAssistant = messages
            ...
            .some(
              (message) =>
                ...
                message.toolCalls.some((toolCall) => toolCall.id === firstToolMessage.toolCallId)
            )
    ...
    const llmClient: LLMClient = {
      chatWithTools: vi
        ...
        .mockResolvedValueOnce({
          ...
          toolCalls: [createToolCall('extract_jobs', { pageType: 'search_results', maxJobs: 1 }, 'tool_extract')]
        })
        .mockResolvedValueOnce({
          content: 'probe one more route before finishing',
          toolCalls: [createToolCall('navigate', { url: 'https://www.linkedin.com/jobs/search/', timeout: 5000 }, 'tool_nav')]
        })
        .mockResolvedValueOnce({
          ...
          toolCalls: [
            createToolCall('finish', {
              reason: 'Enough evidence collected.',
              summary: 'Recommendation routes and filters were proven before ending the phase.',
              ...
              navigationTips: ['Start from /jobs, then open a reusable recommendation collection'],
  ...
  test('forces a final summarize turn and synthesizes partial findings when no finish call is returned', async () => {
    ...
    const llmClient: LLMClient = {
      chatWithTools: vi
        ...
        .mockResolvedValueOnce({
          content: 'extract one sample job first',
          toolCalls: [createToolCall('extract_jobs', { pageType: 'search_results', maxJobs: 1 }, 'tool_extract_partial')]
        })
    ...
    expect(
      (secondCallMessages ?? []).some((message: AgentMessage) =>
    ...
    expect(result.phaseCompletionReason).toContain('timed out before the worker returned a structured finish call')
    expect(result.phaseEvidence?.routeSignals.some((entry) => entry.includes('Job extraction found'))).toBe(true)
  ...
  test('records and recovers from 404-like routes during source-debug phases', async () => {
    ...
    } satisfies Pick<Page, 'goto' | 'waitForTimeout' | 'url' | 'title' | 'locator' | 'evaluate'>
    const llmClient: LLMClient = {
      chatWithTools: vi
        .fn()
        .mockResolvedValueOnce({
          content: 'probe a broken route first',
          toolCalls: [createToolCall('navigate', { url: 'https://www.linkedin.com/jobs/404' }, 'tool_nav_404')]
        })
        .mockResolvedValueOnce({
          ...
          toolCalls: [
            createToolCall('finish', {
              reason: 'Enough evidence collected.',
              summary: 'Recovered from a broken route back to the jobs surface.',
              reliableControls: [],
              trickyFilters: [],
              navigationTips: ['Return to the last known jobs surface after a 404-like route'],
              applyTips: [],
              warnings: []
            }, 'tool_finish_404')
          ]
    ...
    const result = await runAgentDiscovery(page404 as unknown as Page, createConfig(), llmClient, jobExtractor)

    expect(result.phaseCompletionMode).toBe('forced_finish')
    expect(result.phaseEvidence?.warnings.some((entry) => entry.includes('not-found route'))).toBe(true)
    expect(result.phaseEvidence?.routeSignals.some((entry) => entry.includes('Recovered to the last known jobs surface'))).toBe(true)
